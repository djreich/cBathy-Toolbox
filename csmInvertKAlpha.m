    ...
    if ~((lam1Norm>params.minLam) && (maxDx>LExpect) && (maxDy>LExpect))
        ...
        try
            ...
            try
                ...
                kAlphaPhi = levmar(kAlphaPhiInit',...
                    @(x)predictCSM_objective(x,[xy w],[real(v);imag(v)]),...
                    OPTIONS.TolX,OPTIONS.MaxIter);
                [~,~,jacob,resid] = predictCSM_objective(kAlphaPhi, [xy w],[real(v);imag(v)]);
            end
